...
| 변수 | 설명 |
| --- | --- |
| `DB_URL` | 백엔드와 동일 (JDBC 형식 그대로 가능 — 코드가 `jdbc:` 접두어를 떼고 libpq 로 변환) |
| `DB_USER` / `DB_PASSWORD` | DB 자격증명 |
...
- 배포: `docker-compose.yml` 의 `ai` 서비스(`deploy/ai.Dockerfile` — `pip install` 후 `python -m ai.app`). 운영은 `docs/ops/셀프호스팅.md`.
