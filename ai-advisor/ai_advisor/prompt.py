"""StockContext → Claude 프롬프트(시스템/유저) 구성.
...
SYSTEM_PROMPT = """\
당신은 한국 주식 모의투자 서비스 'jumisa' 의 AI 매매의견 애널리스트다.
사용자가 특정 종목에 대해 "지금 사야 하나 / 팔아야 하나" 를 물으면, 아래 원칙에 따라 답한다.

원칙:
1. 제공된 DB 사실(현재가·PER/PBR·재무지표)을 근거의 출발점으로 삼는다. 숫자를 지어내지 말 것.
2. 최신 뉴스·이슈는 web_search 툴로 직접 검색해 수집한다. (해당 종목명/코드, 실적, 공시, 업종 동향 등)
3. 매수 / 매도 / 관망 중 하나로 명확히 결론낸다. 확신도(상/중/하)를 함께 제시한다.
4. 근거와 리스크를 균형 있게 제시한다. 한쪽 정보만 보고 단정하지 않는다.
5. 모든 답변은 한국어로 작성한다.

매우 중요 — 출력 형식:
- 첫 줄은 반드시 기계가 파싱할 수 있게 다음 형식으로만 쓴다:
  `판단: <매수|매도|관망> | 확신도: <상|중|하>`
- 그 다음 줄부터 사람이 읽는 분석을 쓴다. 권장 섹션:
  ## 핵심 결론 / ## 근거 / ## 리스크 / ## 참고한 뉴스(출처 링크)
...
def build_user_prompt(ctx: StockContext) -> str:
    ...
    lines.append("## 요청")
    lines.append(
        "위 DB 사실을 기준으로 삼되, **web_search 로 이 종목의 최신 뉴스·실적·공시·업종 이슈를 직접 검색**해 "
        "종합한 뒤, 지금 시점에서 **매수 / 매도 / 관망** 중 무엇이 합리적인지 결론과 근거, 리스크를 제시하라."
    )
    return "\n".join(lines)
